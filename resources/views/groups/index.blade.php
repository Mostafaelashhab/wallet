<x-layouts.app :title="__('app.groups')">
    <x-header-bar :title="__('app.groups')" />

    <div class="px-5">
        <div class="text-white/85 text-sm mb-3">عندك في {{ $groups->count() }} مجموعة.</div>

        <div class="space-y-3">
            @foreach ($groups as $g)
                <a href="{{ route('groups.show', $g) }}" class="card flex items-center gap-3 tap-anim">
                    <div class="w-12 h-12 rounded-2xl grid place-items-center text-2xl" style="background: {{ $g->color }}22">{{ $g->icon }}</div>
                    <div class="flex-1">
        ...
        <a href="{{ route('groups.create') }}" class="btn-primary mt-6 block text-center">+ مجموعة جديدة</a>
    </div>
</x-layouts.app>
